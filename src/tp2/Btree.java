...
public class Btree {
	...
	public Node searchNode(int i, Node n) {
		...
		for (Valeur v : n.getValeurs()) {
			...
			else if (v.getValeur() == n.getMaxValeur().getValeur() && n.getMaxValeur().getFilsD() == null
					&& n.getMaxValeur().getFilsG() == null) {
				return n;
			}
	...
	public static Valeur split(Valeur v, LinkedList<Valeur> valeurs, Valeur pere, Node nodePere) {
		...
		Node newFilsD = new Node(median, m, new LinkedList<Valeur>(valeurs.subList(valeurs.size() / 2, valeurs.size())),
				nodePere);
